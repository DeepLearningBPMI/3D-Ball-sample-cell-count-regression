...
def run(dataset):
    ...
    for batch in test_generator:
        ...
        def backward_hook(module,grad_in,grad_out):
           ...
           # Ensure previous hooks are removed if running multiple batches
        if hasattr(model.layer4[-1], 'forward_hooks'):
            model.layer4[-1].forward_hooks.clear()
        if hasattr(model.layer4[-1], 'full_backward_hooks'):
            model.layer4[-1].full_backward_hooks.clear()
        ...
        save_dir = os.path.join(activation_dir, dir_name, input_filename)
...
if __name__ == '__main__':
  ...
  activation_dir = code_base + "activation_resnet_0808-1125"
  run(dataset_dir)