...
_DATA_SELECTOR = '*.tif'
classes_name = ['Neutrophils', 'Eosinophils', 'Lymphocytes', 'Monocytes']
batch_size = 1
lr = 0.0012
neuro_hidden =32
dropout =0.3
...
def run(dataset):
    ...
    model.load_state_dict(torch.load('/scistor/guest/mzu240/BAL/Results_Resnet_Blood_LDS_1120/11-20 20:20/resnet3d50_Hep_lr0.0012_dropout0.3_adam_neurons_hidden32/model.pth'))
    model.eval()

    # Create log directory
    result_dir = code_base + '/test-ResNet_blood_11202020_1209'
    now_time = datetime.now()
    time_str = now_time.strftime('%m-%d %H:%M')
    log_dir = os.path.join(result_dir, time_str, f'ResNet3d_test_lr{lr}_dropout{dropout}_11202020')
...
def plot_patient_results(patient, true_labels, predicted_labels, predicted_std):
    ...
    svg_save_path = os.path.join(output_folder, f'{patient}_resnet-11202020.svg')
    tif_save_path = os.path.join(output_folder, f'{patient}_resnet-11202020.tif')
    plt.tight_layout()
...
if __name__ == '__main__':
  ...
  output_folder = os.path.join(code_base, 'predict_plot_2', 'Blood')
  # Create the output folder if it doesn't exist
  if not os.path.exists(output_folder):
    os.makedirs(output_folder)
  csv_filename = 'patient_results_Blood_11202020.csv'
