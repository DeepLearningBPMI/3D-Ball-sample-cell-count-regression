...
def run(dataset):
    ...
    criterion = nn.L1Loss() 
    model.load_state_dict(torch.load('/scistor/guest/mzu240/BAL/Results_3D_1018/10-21 10:20/resnet3d50_Hep_lr1e-05_dropout0.27_SGD_neurons_hidden380/model.pth'))
    print("Model loaded for testing.")
    model.eval()

    #log
    result_dir = code_base + '/test-results-blood'
    now_time = datetime.now()
    time_str = datetime.strftime(now_time, '%m-%d %H:%M')
    log_dir =  os.path.join(result_dir, time_str, f'resnet_test__lr{lr}_dropout{dropout}_10_21_10:20')
